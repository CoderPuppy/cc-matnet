...
function create(opts)
		...
		while true do
			...
			repeat
				...
				elseif _ == 'network' then
					local to = mat.wrap(packet.to)
					local allowed = false
					local allowedBits = 0
					for i = 1, #allowedAddresses do
						local ma, allow = unpack(allowedAddresses[i])
						if ma.matchBits >= allowedBits and mat.match(to, ma) then
							allowedBits = ma.matchBits
							allowed = allow
						end
					end
	...
	function net.close(inter)
		logging.debug('Closing interface: ', inter.name)
	...
	function net.send(packet)
		...
		if mat.match(to, mat.undefined) then
			-- TODO: Stop this from double sending
			for _, inter in pairs(interfaces) do
				inter.send(packet, { inter = inter })
			end
		end
	end
	...
	function net.listen(address)
		address = mat.wrap(address)
		allowedAddresses[#allowedAddresses + 1] = { address, true }
	end
